...
describe("Test Expressions", () => {
  describe("Test visitors", () => {
    ...
    test("Partial visitor", () => {
      ...
      } as Expressions.ExpressionVisitor<string>; // this casting is needed because visitor is partial

      const expr = parseText("2 + 3");
      expect(expr.accept(minimalArithmeticVisitor)).toBe(
        "Right side is number 3\nOperator is addition\nLeft side is number 2",
      );
    });
  });
  ...
  describe("Test metadata getters", () => {
    ...
    test("Test LiteralExpression metadata getters", () => {
      const literalExpr = new Expressions.LiteralExpression({ type: "number", value: 5 });
      expect(literalExpr.type).toBe("number");
